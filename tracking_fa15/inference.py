...
class DiscreteDistribution(dict):
    ...
    def normalize(self):
        ...
        if total != 0:
            for item in self.items():
                self[item[0]] = item[1] / float(total)
    ...
    def sample(self):
        ...
        total = 0
        for item in self.items():
            if rand >= total and rand < total + item[1]:
                return item[0]
            total += item[1]
...
class ExactInference(InferenceModule):
    ...
    def predict(self, gameState):
        ...
        for ghostPosition in self.allPositions:
            ...
            for newGhostPosition, p in newPosDist.iteritems():
                newBeliefs[newGhostPosition] += self.beliefs[ghostPosition] * p
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        legalBoard = self.legalPositions
        lenLegalBoard = len(legalBoard)
        particlesPerSquare = particleCount // lenLegalBoard
        
        for b in legalBoard:
            for p in range(particlesPerSquare + 1):
                self.particles += [b]
    ...
    def update(self, observation, gameState):
        ...
        for particle in self.particles:
            particleDistribution[particle] += self.getObservationProb(observation, pacmanPosition, particle, jailPosition)
        if particleDistribution.total() == 0:
            self.initializeUniformly(gameState)
        else:
            self.particles = [particleDistribution.sample() for _ in range(len(self.particles))]  
...
class JointParticleFilter(ParticleFilter):
    ...
    def initializeUniformly(self, gameState):
        ...
        self.particles = []
        i = 0
        permNum = len(self.legalPositions) * len(self.legalPositions)
        while i < self.numParticles:
            self.particles += itertools.product(self.legalPositions, self.legalPositions)
            i += permNum
    ...
    def update(self, observation, gameState):
        ...
        for particle in self.particles:
            ...
            particleDistribution[particle] += weight
        if particleDistribution.total() == 0:
            self.initializeUniformly(gameState)
        else:
            self.particles = [particleDistribution.sample() for _ in range(self.numParticles)]
